...
// 🔊 Synthesized Audio Engine (Dart Whistle, Board Hit, Rocket Ignition & Fanfare)
const playDartAudio = (type = 'throw') => {
  try {
    ...
    if (type === 'throw') {
      ...
      // High-velocity rocket ignition & thrust whoosh
      const osc = ctx.createOscillator();
      const gain = ctx.createGain();
      osc.type = 'sawtooth';
      ...
      osc.start(now);
      osc.stop(now + 0.5);
      if (navigator.vibrate) navigator.vibrate([50, 80, 120]);
...
// 🌟 STAGE 3: 10 DISCOVERY LENSES (Trending, Close, Cost Effective, Premium, Combos, etc.)
...
export default function DartBoardGame({ selectedCity = 'Alwar', onBack }) {
  // Steps: 1 (Category - 14) | 2 (Subcategory) | 3 (Discovery Lens - 10) | 4 (Matched Deals)
  const [currentStep, setCurrentStep] = useState(1);
  const [isThrowing, setIsThrowing] = useState(false);
  const [isRocketLaunching, setIsRocketLaunching] = useState(false);
  const [boardScore, setBoardScore] = useState(100);
  const [dartHitCoords, setDartHitCoords] = useState({ x: 50, y: 50 });
  ...
  // Moving Laser Crosshair Simulation
  ...
  useEffect(() => {
    if (isThrowing || isRocketLaunching || currentStep === 4) return;

    const loop = () => {
      const elapsed = (Date.now() - timeRef.current) / 1000;
      const x = 50 + Math.sin(elapsed * 2.8) * 26;
      const y = 50 + Math.cos(elapsed * 2.2) * 26;
      setCrosshairPos({ x, y });
  ...
  }, [isThrowing, isRocketLaunching, currentStep]);

  // 🎯 Tap Board: Throw Dart & Shuffle Outcome
  const handleThrowDartOnBoard = () => {
    if (isThrowing || isRocketLaunching) return;
    setIsThrowing(true);
    playDartAudio('throw');

    if (animFrameRef.current) cancelAnimationFrame(animFrameRef.current);

    const hitX = crosshairPos.x;
    const hitY = crosshairPos.y;
    setDartHitCoords({ x: hitX, y: hitY });

    // Calculate score based on distance from center (50, 50)
    const dist = Math.sqrt(Math.pow(hitX - 50, 2) + Math.pow(hitY - 50, 2));
    const score = dist <= 9 ? 100 : dist <= 18 ? 75 : dist <= 28 ? 50 : 25;
    setBoardScore(score);
  ...
  // Lock Selection & Trigger Rocket Dart Launch before Reveal
  const handleAcceptAndNext = () => {
    if (currentStep === 1) {
      playDartAudio('advance');
      ...
      // 🚀 ROCKET DART LAUNCH FROM BOTTOM BAR TO APP BAR
      setIsRocketLaunching(true);
      playDartAudio('rocket');
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between p-3.5 select-none relative overflow-hidden">
      ...
      {/* 🌟 Rocket Dart Launch & Laser Animations */}
      <style>{`
        @keyframes rocketDartFlight {
          0% {
            transform: translate(-50%, 100vh) scale(1.6) rotate(0deg);
            opacity: 0;
          ...
          85% {
            transform: translate(-50%, 4vh) scale(1.2) rotate(-5deg);
            opacity: 1;
          }
          100% {
            transform: translate(-50%, -15vh) scale(0.6) rotate(0deg);
            opacity: 0;
            filter: blur(4px) drop-shadow(0 0 40px rgba(255,255,255,1));
          }
        }
        @keyframes flameBoosterThrust {
          0%, 100% { height: 35px; opacity: 0.9; }
          50% { height: 55px; opacity: 1; filter: brightness(1.6); }
        }
        @keyframes dartStickImpact {
          0% { transform: translate(-50%, -50%) scale(3) rotate(-35deg); opacity: 0; }
          70% { transform: translate(-50%, -50%) scale(1.1) rotate(5deg); opacity: 1; }
          100% { transform: translate(-50%, -50%) scale(1) rotate(0deg); opacity: 1; }
        }
        .rocket-dart-fly-up {
          animation: rocketDartFlight 0.7s cubic-bezier(0.15, 0.85, 0.35, 1) forwards;
        }
        .rocket-booster-flame {
          animation: flameBoosterThrust 0.1s infinite alternate;
        }
        .dart-stick-anim {
          animation: dartStickImpact 0.25s ease-out forwards;
        }
        .vip-gold-pass {
      ...
      {/* 🚀 ROCKET DART OVERLAY: Flown from Bottom Bar to App Bar */}
      {isRocketLaunching && (
        <div className="fixed inset-0 z-50 pointer-events-none overflow-hidden">
          <div className="absolute left-1/2 rocket-dart-fly-up flex flex-col items-center">
            {/* Rocket Dart Head */}
            <div className="text-5xl filter drop-shadow-[0_0_20px_rgba(251,191,36,1)]">
              🎯
            </div>
            {/* Blazing Rocket Booster Flame Tail */}
            <div className="w-4 bg-gradient-to-b from-amber-300 via-orange-500 to-transparent rounded-full rocket-booster-flame -mt-1 shadow-[0_0_25px_rgba(249,115,22,1)]" />
          </div>
        </div>
      )}
      ...
      <header className="flex items-center justify-between z-10 shrink-0 pb-2 border-b border-slate-800">
        ...
        <div className="text-center">
          <span className="text-[9.5px] font-black uppercase text-amber-400 tracking-wider block">
            🎯 TOWN DART BOARD
          </span>
          <span className="text-xs font-black text-slate-100">3-Stage Aim Matcher • {selectedCity}</span>
        </div>
      ...
      <div className="grid grid-cols-3 gap-1.5 z-10 py-2.5 shrink-0">
        ...
        >
          ...
          <span className="text-xs font-black text-slate-100 truncate block mt-0.5">
            {lockedCategory ? `${lockedCategory.icon} ${lockedCategory.label.split(' ')[0]}` : 'Throw Dart...'}
          </span>
        </div>
      ...
      {/* ========================================================================= */}
      {/* 🌟 THROWING STAGE (STEPS 1, 2 & 3: TAP BOARD TO AIM & FIRE)               */}
      {/* ========================================================================= */}
      {currentStep <= 3 && (
        <div className="relative flex-1 flex flex-col items-center justify-between my-auto py-2 z-10">
          
          <div className="text-center space-y-1">
            <span className="text-[10px] font-black uppercase px-3 py-0.5 rounded-full bg-slate-900 border border-slate-700 text-amber-300">
              {currentStep === 1
                ? 'Step 1 of 3: Throw Dart for 14 Categories'
                : currentStep === 2
                ? `Step 2 of 3: Throw Dart for ${lockedCategory?.label} Subcategories`
                : 'Step 3 of 3: Throw Dart for Discovery Lens (Trending, Budget, Close)'}
            </span>
            <h2 className="text-base font-black text-slate-100 tracking-tight pt-0.5">
              Tap the Dart Board to Fire!
            </h2>
            <p className="text-xs text-slate-400">
              Throw freely until you hit the outcome you desire, then accept & continue.
            </p>
          </div>

          {/* 🎯 THE INTERACTIVE DART BOARD */}
          <div
            onClick={handleThrowDartOnBoard}
            className="relative w-68 h-68 rounded-full bg-gradient-to-b from-stone-900 via-stone-950 to-black border-4 border-amber-400/80 shadow-[0_0_40px_rgba(0,0,0,0.9),inset_0_0_25px_rgba(245,158,11,0.2)] p-2 flex items-center justify-center cursor-pointer select-none group my-auto active:scale-95 transition"
          >
            {/* Outer Ring (25 Pts) */}
            <div className="relative w-full h-full rounded-full border-4 border-purple-500/40 bg-gradient-to-tr from-slate-950 via-purple-950/30 to-slate-950 flex items-center justify-center">
              
              {/* Middle Ring (50 Pts) */}
              <div className="w-44 h-44 rounded-full border-4 border-cyan-400/50 bg-gradient-to-br from-slate-950 via-cyan-950/40 to-slate-950 flex items-center justify-center shadow-inner">
                
                {/* Inner Ring (75 Pts) */}
                <div className="w-28 h-28 rounded-full border-4 border-red-500/70 bg-gradient-to-tr from-slate-950 via-red-950/60 to-slate-950 flex items-center justify-center shadow-lg">
                  
                  {/* 🌟 Bullseye Center (100 Pts) */}
                  <div className="w-14 h-14 rounded-full bg-gradient-to-br from-amber-300 via-yellow-400 to-amber-600 border-4 border-amber-200 shadow-[0_0_25px_rgba(251,191,36,0.9)] flex items-center justify-center animate-pulse">
                    <span className="text-slate-950 text-xs font-black font-mono">100</span>
                  </div>
                </div>
              </div>
            </div>

            {/* 🎯 Moving Laser Crosshair (Aiming) */}
            {!isThrowing && (
              <div
              ...
              >
                <div className="w-10 h-10 rounded-full border-2 border-red-500 border-dashed animate-spin shadow-[0_0_12px_rgba(239,68,68,0.9)]" />
                <div className="absolute w-2.5 h-2.5 rounded-full bg-red-400 shadow-[0_0_8px_rgba(239,68,68,1)] animate-ping" />
              </div>
            )}

            {/* 🎯 Hit Dart Stuck in Board */}
            {isThrowing && (
              <div
                style={{ left: `${dartHitCoords.x}%`, top: `${dartHitCoords.y}%` }}
                className="absolute z-30 dart-stick-anim flex flex-col items-center pointer-events-none"
              >
                <div className="text-3xl filter drop-shadow-[0_4px_8px_rgba(0,0,0,0.9)]">🎯</div>
              </div>
            )}
          </div>

          {/* Current Settled Outcome Ping Card */}
          <div className="w-full max-w-xs bg-gradient-to-b from-slate-900 via-slate-950 to-slate-900 border-2 border-amber-400/60 rounded-3xl p-3.5 text-center shadow-2xl space-y-2">
          ...
          <div className="w-full max-w-xs grid grid-cols-2 gap-2 pt-2">
            <button
              type="button"
              onClick={handleThrowDartOnBoard}
              disabled={isThrowing || isRocketLaunching}
              className="py-3 rounded-2xl bg-slate-900 hover:bg-slate-800 text-amber-300 border border-amber-400/50 font-black text-xs uppercase tracking-wider shadow-lg active:scale-95 transition cursor-pointer flex items-center justify-center space-x-1"
            >
              <span>🎯</span>
              <span>Re-throw (फिर फेंकें)</span>
            </button>
            ...
            >
              <span>{currentStep === 3 ? '🚀 Launch Match' : '✓ Accept (सही है)'}</span>
              <span>➔</span>
            </button>
          </div>
        </div>
        ...
        <div className="relative flex-1 flex flex-col justify-between z-10 py-1 space-y-3">
              ...
              return (
                ...
                >
                  ...
                  <ActionButtons
                    ...
                    message={`Namaste! Maine TownHub Dart Board Match mein aapka offer "${item.title || item.name}" (${lockedSubcategory?.label} • ${lockedDiscoveryLens?.label}) dekha. Kya yeh abhi available hai?`}
                  />
                </div>
              );
            })}
          ...
          <div className="pt-1 shrink-0">
            <button
              type="button"
              onClick={handleRestart}
              className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-amber-400 via-yellow-400 to-amber-500 text-slate-950 font-black text-xs uppercase tracking-wider shadow-xl active:scale-95 transition cursor-pointer"
            >
              🎯 Throw Dart Again (नया डार्ट मैच बनाएं)
            </button>
          </div>
        </div>
      )}
